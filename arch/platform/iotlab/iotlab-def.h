...
/* Modern core os/sys/clock.h now provides the clock_time_t typedef itself,
 * sized by CLOCK_CONF_SIZE (default 8/uint64_t) -- this platform's timing
 * code (clock-systick.c, rtimer-arch.c) was written for the original
 * 32-bit width, so keep that explicitly rather than let 64-bit arithmetic
 * leak into code that never expected it. */
